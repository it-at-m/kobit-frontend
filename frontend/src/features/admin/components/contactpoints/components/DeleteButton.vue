<template>
  <v-row justify="center">
    <v-btn color="warning" :loading="isLoading" :disabled="isLoading" @click="openDialog">
      <v-icon>mdi-delete</v-icon> Löschen
    </v-btn>
    <v-dialog v-model="isDialogActive" persistent max-width="590">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="green darken-1" text @click="isDialogActive = false">
            Abbrechen
          </v-btn>
          <v-btn color="green darken-1" text @click="deleteContactPoint">
            <v-icon>mdi-delete</v-icon> Löschen
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-snackbar v-model="isSnackbarActive" :timeout="SNACKBAR_TIMEOUT" color="green darken-1" bottom>
      <p class="pa-0 ma-0">Anlaufstelle erfolgreich gelöscht! <v-icon>mdi-check</v-icon></p>
    </v-snackbar>
  </v-row>
</template>
