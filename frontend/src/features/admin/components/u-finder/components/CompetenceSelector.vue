...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(() => {
      editableContactPointList.value = props.convo.contactPoints.map(
        (it: ContactPoint) => {
          return {
            id: it.id,
            name: it.name,
      ...
      editableContactPointList.value = editableContactPointList.value.sort(
        (cp1, cp2) => cp1?.position - cp2?.position
      );
    });

    watch(listItems, (newValue) => {
      if (newValue) {
        const presentContactPointIds = editableContactPointList.value.map(
          (it: ContactPointListItem) => it.id
        );
        availableContactPoints.value = newValue.filter(
          (it: ContactPointListItem) => !presentContactPointIds.includes(it.id)
        );
      }
    });

    const handleRemoveContactPoint = (contactPoint: ContactPointListItem) => {
      editableContactPointList.value = editableContactPointList.value.filter(
        (it) => it.id !== contactPoint.id
      );
    ...
    const handleAddContactPoint = (contactPoint: ContactPointListItem) => {
      ...
      availableContactPoints.value = availableContactPoints.value.filter(
        (it) => it.id !== contactPoint.id
      );
    };
    ...
    const handlePositionChange = (
      newIndex: number,
      oldIndex: number,
      element: ContactPointListItemWithPosition
    ) => {
      const elementToMove = {
        ...editableContactPointList.value[newIndex - 1],
        position: oldIndex,
      };
      ...
      const a = temp.filter(
        (it) => it.id !== elementToMove.id && it.id !== elementTriggered.id
      );
    ...
    const save = () => {
      const pathCompetences = props.givenAnswers.map(
        (x: QuestionAndAnswer) => x.answerCompetence
      );
      const competenceList: ListItemToCompetenceView[] =
        editableContactPointList.value.map(
          (it: ContactPointListItemWithPosition) => {
            return {
              listItem: it,
              competences: pathCompetences,
              position: it.position,
            } as ListItemToCompetenceView;
          }
        );
      mutateAsync(competenceList)
        .then(() => {
          if (isSuccess) {
            ...
            snackBarText.value =
              "Anlaufstellen wurden erfolgreich dem Unterstützungsfinderpfad hinzugefügt.";
        ...
        .catch(() => {
          showSnackBar.value = true;
          snackBarText.value =
            "Beim Speichern gab es ein Problem. Versuchen Sie es bitte erneut oder wenden Sie sich an die Betreiber der Anwendung.";
