...
export const adminBaseHeadLineLabels = new I18nLabelMaker(
  adminBaseHeadline,
  AdminHeadlineLabels
).labels();
...
const adminInformation = new Map<string, string>([
  ...
  [
    AdminInformationLabels.DEPARTMENT_ADMIN,
    `${i18n.t(`${DEPARTMENT_ADMIN_PATH}`)}`,
  ],
  [AdminInformationLabels.DEPARTMENT, `${i18n.t(`${DEPARTMENT_PATH}`)}`],
  [AdminInformationLabels.NO_ADMIN, `${i18n.t(`${NO_ADMIN_PATH}`)}`],
  [
    AdminInformationLabels.INFORMATION_TITLE,
    `${i18n.t(`${INFORMATION_TITLE_PATH}`)}`,
  ],
]);

export const adminInformationLabels = new I18nLabelMaker(
  adminInformation,
  AdminInformationLabels
).labels();
