  ...
  <v-row>
    <v-dialog v-model="dialog" max-width="1000px" @click:outside="closeDialog">
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-form v-model="isFormValid">
              <v-row>
                <v-col cols="12">
                  <v-text-field v-model="addedItem.header" :label="headerLabel" :rules="[headerRule]"
                    max-file-name-input-length="250" counter />
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">
                  <v-textarea v-model="addedItem.entry" :label="entryLabel" :rules="[entryRule]"
                    max-file-name-input-length="1500" counter />
                </v-col>
              </v-row>
              <v-row v-if="props.pageType === 'DOWNLOADS'">
                <v-col cols="12">
                  <v-file-input v-model="file" :rules="fileRules" accept=".pdf,.doc,.docx,.odf,.PDF,.DOC,.DOCX,.ODF"
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="error" @click="cancelAdd">
            <v-icon>mdi-cancel</v-icon> Abbruch
          </v-btn>
          <v-btn color="success" :loading="isLoading" :disabled="!isFormValid || isLoading"
            @click="() => saveAdd(file)">
