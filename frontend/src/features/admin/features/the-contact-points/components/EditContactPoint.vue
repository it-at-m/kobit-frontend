                  ...
                  <a
                    ...
                    >{{
                      writableContactPoint.image
                        ? getFileNameFromLink(
                            writableContactPoint.image.toString()
                          )
                        : ""
                    }}</a
                  >
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const addNewContact = (value: Contact) => {
      const newContact = {
        ...value,
        contactPointId: props.listItem?.id,
      } as Contact;
    ...
    const computeMarkdown = computed(() =>
      marked.parse(writableContactPoint.value?.description || "")
    );
    ...
    function removeLink(item: Link) {
      if (writableContactPoint.value?.links) {
        const links = writableContactPoint.value.links.filter(
          (it) => it !== item
        );
        writableContactPoint.value = {
          ...writableContactPoint.value,
    ...
    function removeContact(contact: Contact) {
      if (writableContactPoint.value?.contact) {
        const contacts = writableContactPoint.value.contact.filter(
          (it) => it !== contact
        );
        writableContactPoint.value = {
          ...writableContactPoint.value,
