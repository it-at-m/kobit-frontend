...
export const theDisclaimerStoreModule = {
  ...
  state: {
    ...
    disclaimerMessage:
      "Auf diesen Seiten werden keine Informationen gespeichert. Alle getätigten Eingaben werden rein zu Darstellungszwecken verarbeitet.",
  } as TheDisclaimerStoreState,
  mutations: {
    [THE_DISCLAIMER_MUTATION_SET_IS_ACTIVE](
      state: TheDisclaimerStoreState,
      payload: boolean
    ): void {
  ...
  getters: {
    [THE_DISCLAIMER_GETTER_GET_IS_ACTIVE](
      state: TheDisclaimerStoreState
    ): boolean {
    ...
    [THE_DISCLAIMER_GETTER_GET_DISCLAIMER_MESSAGE](
      state: TheDisclaimerStoreState
    ): string {
