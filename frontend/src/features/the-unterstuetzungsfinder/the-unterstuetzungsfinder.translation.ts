...
export const finderMap = new Map<string, string>([
  ...
  [
    FinderLabels.POINT_OF_CONTACTS,
    `${i18n.t(`${TEXTS}.${FinderLabels.POINT_OF_CONTACTS}`)}`,
  ],
  [FinderLabels.ANSWERS, `${i18n.t(`${TEXTS}.${FinderLabels.ANSWERS}`)}`],
  [
    FinderLabels.BANNER_TEXT,
    `${i18n.t(`${BANNER}.${FinderLabels.BANNER_TEXT}`)}`,
  ],
  [
    FinderLabels.BANNER_CLOSE,
    `${i18n.t(`${BANNER}.${FinderLabels.BANNER_CLOSE}`)}`,
  ],
]);

export const finderLabels = new I18nLabelMaker(
  finderMap,
  FinderLabels
).labels();
