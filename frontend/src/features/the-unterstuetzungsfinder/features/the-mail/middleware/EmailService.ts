...
import {
  getUserMailAddress,
  sendMail,
} from "@/features/the-unterstuetzungsfinder/features/the-mail/api/MailClient";
import { Email } from "@/features/the-unterstuetzungsfinder/features/the-mail/types/Email";

export const useGetMailAddress = () => {
  const { isLoading, isError, data, error } = useQuery(["mail"], () =>
    getUserMailAddress()
  );
...
export const useSendMail = (email: Email) => {
  const { isLoading, isError, isSuccess } = useQuery(["sendMail", email], () =>
    sendMail(email)
  );
