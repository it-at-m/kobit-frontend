  ...
  methods: {
    async download() {
      ...
      for (let i = 0; i < decisionTree.length; i++) {
        pdf.text(
          i + 1 + ". Frage: " + decisionTree[i].questionAnswered,
          80,
          30 + pdfSpacer
        );
        pdf.text(
          "Ihre Antwort: " + decisionTree[i].answerValue,
          80,
          30 + (pdfSpacer + 18)
        );
      ...
      pdf.text(
        "Für Ihr Anliegen werden folgende Anlaufstellen vorgeschlagen:",
        80,
        50 + pdfSpacer
      );
      ...
      for (let i = 0; i < convo.contactPoints.length; i++) {
        pdf.text(
          convo.contactPoints[i].shortCut + ": " + convo.contactPoints[i].name,
          80,
          75 + pdfSpacer
        );
        if (convo.contactPoints[i].contact[0]) {
          pdf.text(
            "Kontakt: " + convo.contactPoints[i].contact[0].email,
            80,
            75 + (pdfSpacer + 18)
          );
        } else {
      ...
      pdf.textWithLink(
        "https://kobit.muenchen.de/#/erfahre-mehr",
        537,
        100 + pdfSpacer,
        { url: "https://kobit.muenchen.de/#/erfahre-mehr" }
      );
